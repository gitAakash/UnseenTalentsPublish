    ...
    function $http(requestConfig) {
      ...
      promise.success = function (fn) {
          promise.then(function (response) {
              if (response.headers('X-Responded-JSON')) {
                  var respondedJson = response.headers('X-Responded-JSON');
                  var objrespondedJson = $.parseJSON(respondedJson);
                  if (objrespondedJson.status == 401) {
